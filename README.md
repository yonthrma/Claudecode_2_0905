...
`.claude/skills/` 에 프로젝트 진행용 스킬 10개, `.claude/agents/` 에 라우팅 에이전트 1개가 들어 있습니다.
이 저장소 안에서는 바로 쓸 수 있고, 다른 프로젝트로 복사하는 방법은
[.claude/skills/README.md](.claude/skills/README.md) 에 있습니다.

**뭘 쓸지 모르겠으면 `/route` 하나만 기억하면 됩니다.** 나머지는 알아서 골라 씁니다.

| 스킬 | 부르는 법 | 하는 일 |
| --- | --- | --- |
| **route** | `/route <요청>` | **어떤 스킬을 어떤 순서로 쓸지 알아서 판단해서 진행** |
...
└── .claude/
    ├── agents/
    │   └── dispatcher.md   스킬 조합을 판단하는 에이전트
    └── skills/             개발 스킬 묶음
        ├── README.md
        ├── route/SKILL.md
        ├── set-goal/SKILL.md
